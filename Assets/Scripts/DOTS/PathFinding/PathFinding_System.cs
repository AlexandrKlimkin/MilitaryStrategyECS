...
public class PathFinding_System : SystemBase
{
    ...
    protected override void OnUpdate()
    {
        ...
        int i = 0;
        Entities.WithNone<Unit_Routed>().WithoutBurst().WithStructuralChanges().ForEach(
            (Entity e, ref PathFinding_Component uc, ref DynamicBuffer<Unit_Buffer> ub) =>
            {
                if (i <= UnitManager.instance.maxEntitiesRoutedPerFrame)
                {
                    int fromKey = ((int) uc.fromLocation.x + (int) uc.fromLocation.y + (int) uc.fromLocation.z) *
                                  UnitManager.instance.maxPathSize;
                    int toKey = ((int) uc.toLocation.x + (int) uc.toLocation.y + (int) uc.toLocation.z) *
                                UnitManager.instance.maxPathSize;
                    ...
                    {
                        NavMeshQuery currentQuery = new NavMeshQuery(navMeshWorld, Allocator.Persistent,
                            UnitManager.instance.maxPathNodePoolSize);
        ...
        foreach (JobHandle jh in jobHandles)
        {
            if (statusOutputs[j][0] == 1)
            {
                ...
                PathFinding_Component uc = EntityManager.GetComponentData<PathFinding_Component>(routedEntities[j]);
                uc.routed = true;
                EntityManager.SetComponentData<PathFinding_Component>(routedEntities[j], uc);
        ...
        //Movement
        Entities
            .WithAll<Unit_Routed>().ForEach(
                (ref PathFinding_Component uc, ref DynamicBuffer<Unit_Buffer> ub, ref Translation trans) =>
                {
                            ...
                            uc.currentBufferIndex < ub.Length - 1)
                        {
                            ...
                            if (uc.currentBufferIndex == ub.Length - 1)
                            {
                                uc.reached = true;
                                 ...
                                 math.distance(trans.Value, ub[uc.currentBufferIndex].wayPoints) <=
                                 uc.minDistanceReached && uc.currentBufferIndex > 0)
                        {
                            uc.currentBufferIndex -= 1;
